...
def getClass(text):
	...
	if classNum == 0x1: #object references
		...
		objName = ''
		if classNum in names.atom_class_names:
			objName = names.atom_class_names[classNum] + '(' + str(classNum) + ')'
		elif classNum in backup_class_names.atom_class_names:
			objName = backup_class_names.atom_class_names[classNum] + '(' + str(classNum) + ')'
		else:
